...
class PicturePointsSelectPanel(wx.Panel):
   ...
   def getOffsets(self):
        print("-")
        # offsetsFile = open(curdir + "/picturepointspassword.txt", "r")
        # for i in range(4):
   ...
   def onClick(self, event):
       ...
       if (len(self.selection) == 4):
           ...
           print("GOT: ", self.selection)
           #for i in range(4):

'''
           if (password bad):
               incorrectPopup = wx.MessageDialog(None, "The password entered does not match the one stored", " ", wx.RETRY | wx.QUIT | wx.ICON_INFORMATION)
                 if happySelection.ShowModal() == wx.ID_QUIT: close
                 if happySelection.ShowModal() == wx.ID_RETRY: reset
